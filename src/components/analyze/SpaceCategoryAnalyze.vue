...
import { getSpaceCategoryAnalyzeApi } from '@/api'
...
const fetchData = async () => {
  loading.value = true
  const res = await getSpaceCategoryAnalyzeApi({
...
const options = computed(() => {
  const categories = dataList.value.map((item) => item.categoryDTO?.name ?? '未分类')
